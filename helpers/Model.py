...
from . import Corpus, CDBSCAN, Cosine
from .Terms import CoreConceptValue


Reducers    = Literal["none", "pca", "umap"]
Clusterers  = Literal["dbscan", "c-dbscan","cosine","kmeans"]
...
class Model():
    ...
    def iterate(self, X, y, nb_iter = 5, verbose = 0):
        ...
        for i in range(nb_iter):
            ...
            if self.clusterer_type == "kmeans":
                print("nb of iterations:",self.clusterer.n_iter_)
                cc_term_indices = [self.corpus.vocabulary.get_index(term) for term in self.corpus.vocabulary.core_concepts]
                print(self.clusterer.labels_[cc_term_indices])
            # print the number of diffent labels if classifier=dbscan    
            
            if self.clusterer_type == "dbscan":
                print("nb of different labels:")
                print(len(set(self.y_pred)))
            # print  nb of labeled terms
            print(len(self.y_pred[self.y_pred != -1]))
    ...
    def __classify(self, X, y):
        ...
        # X_train_ = []
        # y_train_ = []
        # for i,term in enumerate(self.corpus.vocabulary.terms):
        #     if term in self.corpus.vocabulary.core_concepts or term == "other" or y[i] == -1:
        #         print(term)
        #         continue
    ...
    def __set_reducer(self):
        ...
        if self.reducer_type == "umap":
            ...
            # self.reducer = UMAP(n_components = self.dimensions, n_neighbors=50, min_dist=0.02, random_state=42) #(50,0.02) 0.407 0.511 0.407 0.423
            # self.reducer = UMAP(n_components = self.dimensions, n_neighbors=128, min_dist=0.2, random_state=42) # (128,0.2) 0.410 0.511 0.410 0.428
            self.reducer = UMAP(n_components = self.dimensions, n_neighbors=128, min_dist=0.1, random_state=42) # w/o "pseudo anoonymous data"
            # self.reducer = UMAP(n_components = self.dimensions, n_neighbors=128, min_dist=0.02, random_state=42) # 15th w/o subsub concepts
            # self.reducer = UMAP(n_components = self.dimensions, n_neighbors=128, min_dist=0.05, random_state=42) # 3,4th w/o subsub concepts
            # self.reducer = UMAP(n_components = self.dimensions, n_neighbors=128, min_dist=0.02, random_state=42) # 3,4th w/o subsub concepts
            return
    ...
    def __store_clustering_result(self, X=None, predicted_labels=None):
        ...
        print("nb of different labels:",len(set(predicted_labels)))
    ...
    def __reorder_confusion_matrix(self, matrix=[]):
        ...
        """
        # select each row's max value columns to assign the true label
        major_labels = np.argmax(matrix,axis=1)
        # s = None
        # def _make_cost_m(cm):
        #     s = np.max(cm)
        #     return (- cm + s)
    ...
    def __show_prediction(self, y_pred):
        ...
        # colormap = ["#003a7d","#008dff","#ff73b6","#c701ff","#4ecb8d","#ff9d3a","#f9e858","#d83034"][::-1]+[plt.cm.get_cmap('Spectral')(i / len(self.core_concept_indices + [other_index])) 
        #                   for i in range(7, 7 + len(self.core_concept_indices + [other_index]))]
        # cc_index = [ self.corpus.vocabulary.get_at(cc).get_core_concept() for cc in self.core_concept_indices.tolist()+[other_index]]
